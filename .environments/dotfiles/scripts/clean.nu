...
# Clean up Nix (excluding project environments)
export def main [
...
# Clean all profiles (including project environments)
def "main all" [
...
# Clean all old roots (excluding project environments)
def "main old" [] {
  if (is-nixos) {
...
# Clean the current user's profile (including project environments)
def "main user" [
  --keep: string # At least keep this number of generations
  --keep-since: string # At least keep gcroots and generations in this time range since now
] {
